...
import static dr.manhattan.external.api.WebWalker.walker_engine.real_time_collision.CollisionDataCollector.getCollisionData;
...
public class Reachable {
    ...
    private Reachable(RSTile homeTile) {
        map = generateMap(homeTile != null ? homeTile : new RSTile(MPlayer.location()));
    }
    ...
    public ArrayList<RSTile> getPath(int x, int y) {
        ...
        RSTile playerPos = new RSTile(MPlayer.location()).toLocalTile();
        ...
        RSTile tile = new RSTile(x, y, MPlayer.location().getPlane(), RSTile.TYPES.LOCAL);
    ...
    private static RSTile convertToLocal(int x, int y) {
        RSTile position = new RSTile(x, y, MPlayer.location().getPlane(), x >= 104 || y >= 104 ? RSTile.TYPES.WORLD : RSTile.TYPES.LOCAL);
    ...
    public static Reachable getMap() {
        return new Reachable(new RSTile(MPlayer.location()));
    }
